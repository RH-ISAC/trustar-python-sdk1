...
def main():
    ...
    if do_add_enclave_tag:
        ...
        try:
            ...
            response = ts.get_all_enclave_tags(enclave_ids=ts.enclave_ids)
            print("List of enclave tags for enclave %s\n" % enclave_id)
            print(json.dumps(response, indent=2))

            # Search report by tag
            results = ts.get_report_iterator(from_time=yesterday_time, to_time=current_time, enclave_ids=ts.enclave_ids,
                                             tag="triage")
            # print("Got %s results" % (results.get('totalElements')))
